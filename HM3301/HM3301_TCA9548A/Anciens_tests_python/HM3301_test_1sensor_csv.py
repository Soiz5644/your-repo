...
class HM3301:
    ...
    def get_data(self, select):
        ...
        time.sleep(0.005)  # Petite pause pour laisser le temps à la communication
...
# Temporisation pour l'initialisation du capteur
time.sleep(30)

try:
    # En-tête du CSV
    log_to_csv(["Timestamp", "PM1.0", "PM2.5", "PM10"])
    ...
    while True:
        ...
        if std_PM1 is not None and std_PM2_5 is not None and std_PM10 is not None:
            ...
            log_to_csv([time.strftime("%Y-%m-%d %H:%M:%S"), std_PM1, std_PM2_5, std_PM10])
        else:
