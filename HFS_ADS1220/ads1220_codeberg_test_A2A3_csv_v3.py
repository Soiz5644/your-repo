...
VREF = 3.3      # Référence externe 3.3V (AVDD)
...
def config_ADC_custom(adc, gain):
    ...
    # VREF externe analogique (AVDD/AVSS, donc 3.3V), pas d'IDAC, pas de FIR
    cfg2 = (adc.CFG2_IDAC_OFF
            | adc.CFG2_PSW_ALWAYS_OPEN
            | adc.CFG2_FIR_NONE
            | adc.CFG2_VREF_EXT_ANALOG)
...
print(f"Configuration ADS1220 : AIN2–AIN3, VREF={VREF}V (AVDD), gain={GAIN}, DRDY=GPIO{DRDY}")
...
with open(csv_filename, mode='w', newline='') as csvfile:
    ...
    try:
        while True:
            voltage, gain, value = adc.read_adc_voltage()
