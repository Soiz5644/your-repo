...
const std::string influxdb_host = "00.00.00.0"; // Replace with your InfluxDB host
...
void writeToInfluxDB(const std::string& measurement, int pga, int32_t ain0_raw, int32_t ain1_raw, double ain0_scale, double ain1_scale, int32_t ain0_ain1_raw, double ain0_ain1_scale) {
    CURL *curl;
    CURLcode res;
...
int main() {
    ...
    while (true) {
        for (uint8_t pga : pgaValues) {
            ...
            usleep(50000); // Wait for configuration to take effect

            // Start conversion for AIN0
            spiWriteByte(CMD_START);
            usleep(50000); // Wait for conversion to complete
            int32_t ain0_raw = readADCData();
            double ain0_scale = (ain0_raw / 8388607.0) * (vRef / (1 << pga));
            ...
            spiWriteByte(CMD_START);
            usleep(50000); // Wait for conversion to complete
            int32_t ain1_raw = readADCData();
            double ain1_scale = (ain1_raw / 8388607.0) * (vRef / (1 << pga));
            ...
            double ain0_ain1_scale = (ain0_ain1_raw / 8388607.0) * (vRef / (1 << pga));

            // Log data
            std::string timestamp = getCurrentTimestamp();
            file << timestamp << "," << (1 << pga) << ","    // Timestamp and PGA
                 << ain0_raw << "," << ain1_raw << ","       // AIN0_raw and AIN1_raw
                 << ain0_scale << "," << ain1_scale << ","   // AIN0_scale and AIN1_scale
                 << ain0_ain1_raw << "," << ain0_ain1_scale  // AIN0-AIN1_raw and AIN0-AIN1_scale
                 << "\n";
            file.flush(); // Ensure data is written to the file

            // Write data to InfluxDB
            writeToInfluxDB("ads1220_data", (1 << pga), ain0_raw, ain1_raw, ain0_scale, ain1_scale, ain0_ain1_raw, ain0_ain1_scale);
        }
