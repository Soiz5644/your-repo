...
SPI_BUS = 0
SPI_DEVICE = 0
SPI_SPEED_HZ = 500000  # 500 kHz - vitesse basse pour robustesse
...
def ads1220_read_all_regs(spi):
    spi_cs_low()
    try:
        spi.xfer2([CMD_RREG | (REG_CONF0 << 2), 0x03])  # Lecture de 4 registres
        val = spi.xfer2([0x00] * 4)
    finally:
        spi_cs_high()
    return val

def ads1220_read_reg(spi, reg):
    spi.xfer2([CMD_RREG | (reg << 2), 0x00])
    time.sleep(0.001)
    val = spi.xfer2([0x00])[0]
    print(f"[DEBUG] Read register {reg}: {bin(val)} (0x{val:02X})")
    return val
...
def ads1220_init(spi, gain):
    ...
    # Commande de lecture vide pour forcer la sortie du reset (synchronisation)
    spi_xfer_with_cs(spi, [CMD_START])
    time.sleep(0.01)
    spi_xfer_with_cs(spi, [CMD_POWERDOWN])
    time.sleep(0.01)
    ...
    reg1 = 0b10000000  # Data rate=20SPS, mode normal, conversion single-shot
...
#    read_back = [ads1220_read_reg(spi, i) for i in range(4)]
#    print("[DEBUG] Lecture directe : ", read_back)
...
def main():
    ...
    spi.mode = 0  # Mode SPI 1 (CPOL=0, CPHA=1) pour ADS1220
