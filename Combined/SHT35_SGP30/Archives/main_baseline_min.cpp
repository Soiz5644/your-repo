...
#define FIRST_STARTUP_FILE "first_startup.txt"
...
// Function to check if it is the first startup
bool is_first_startup() {
    std::ifstream infile(FIRST_STARTUP_FILE);
    return !infile.good();
}

// Function to mark that the first startup has occurred
void mark_first_startup() {
    std::ofstream outfile(FIRST_STARTUP_FILE);
    if (outfile.is_open()) {
        outfile << "This file marks that the first startup has occurred.";
        outfile.close();
    }
}
...
void write_baseline(uint16_t co2, uint16_t tvoc) {
    log_debug_info("Appending baseline to file.");
    std::ofstream outfile(BASELINE_FILE, std::ios::app);
    if (outfile.is_open()) {
        // Get current time
        std::time_t now = std::time(nullptr);
        std::tm* local_time = std::localtime(&now);
        ...
        outfile << "[" << time_str << "] CO2: " << co2 << ", TVOC: " << tvoc << std::endl;
        outfile.close();
        log_debug_info("Baseline appended to file: CO2 = " + std::to_string(co2) + ", TVOC = " + std::to_string(tvoc));
    } else {
        log_debug_info("Failed to open baseline file for appending.");
    }
}
...
int main() {
    ...
    // Start IAQ measurement
    sgp30_iaq_init();

    // Check if it is the first startup
    if (is_first_startup()) {
        uint16_t tvoc_inceptive_baseline;
        if (sgp30_get_tvoc_inceptive_baseline(&tvoc_inceptive_baseline) == 0) {
            sgp30_set_tvoc_baseline(tvoc_inceptive_baseline);
            log_debug_info("Inceptive baseline set: TVOC = " + std::to_string(tvoc_inceptive_baseline));
        } else {
            log_debug_info("Failed to retrieve or set inceptive baseline.");
        }
        mark_first_startup();
    }
